...
public class Controllo {
	public static void main(String[] args) {
		...
		String cognome = "Piccione";
		String nome = "Riccardo";
		String dataNascita = "01/11/1985";
		String comune = "Milano";
		String sesso = "M";
		String codFis = "";
		
		/* calcolo le prime 3 lettere del cognome */
		int cont = 0;
		cognome = cognome.toUpperCase();
		nome = nome.toUpperCase();

		/* nel caso il cognome sia più corto di 3 lettere */
		if (cognome.length() < 3) {
			codFis += cognome;
		...
		/* lettere nome */
		cont = 0;
		/* nel caso il nome sia più corto di 3 lettere */
		if (nome.length() < 3) {
			codFis += nome;
		...
		/* inseriamo ora l'anno di nascita*/
		codFis += dataNascita.substring(8, 10);
		
		/* inseriamo ora il mese di nascita */
		int mese = 0;
		if (dataNascita.charAt(3) == '0')
			mese = Integer.parseInt(dataNascita.substring(4, 5));
		else
			mese = Integer.parseInt(dataNascita.substring(3, 5));
		switch (mese) {
		case 1: {
			codFis += "A";
			break;
		...
		/* e infine il giorno della data di nascita */
		int giorno = 0;
		if (dataNascita.charAt(0) == '0')
			giorno = Integer.parseInt(dataNascita.substring(0, 1));
		else
		...
		/* inseriamo il comune di nascita */

		Connection c;
		try {
			c = DriverManager.getConnection("jdbc:mysql://localhost/codfiscale?useSSL=false", "root", "root");
			Statement s = (Statement) c.createStatement();
			ResultSet comuni = s.executeQuery("SELECT codFisco FROM comuni WHERE Comune = '" + comune +"'" );
